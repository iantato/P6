...
func shoot(blocks_to_change: Array):
	for tile in blocks_to_change:
		print(tile)
		lasermap.set_cell(tile, atlas_id, laser_atlas.get(color))  # Add laser tile
	lasermap.set_cell(blocks_to_change[-1] + Vector2i(0, 1), atlas_id, laser_atlas.get(color) + Vector2(0, 1))
